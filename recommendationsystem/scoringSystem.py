...
class scroingSystemForWebsite:
	...
	def getScores(self,searchParams,pastPropAttrList,recoPropAttrList,preferanceList ):
		...
		self.filterArray = [1]*(len(recoPropAttrList))
		""" Location"""
...
# 				print i, b , a[b]
	...
	def getBHKScore(self,searchParams,pastPropAttrList,recoPropAttrList):
		...
		text = ""
		bhkScoreList = []
		for i,bhk in enumerate(recoBHK):
			if searchBHK == -1:
				if bhk == -1 or bhk == 0:
					score =  self.scoringMax
					text = "Congrats, its what you wanted"
					flag =1
				else:
					self.filterArray[i] = 0
	...
	def getBudgetScore(self,searchParams,pastPropAttrList,recoPropAttrList):
		...
		for enumi,price in enumerate(recoBudget):
			...
			if(searchBudget >= self.budgetMaxInput):
				if(price >= searchBudget):
					score = self.scoringMax
					text = "Congrats, the Property meets your budget requirements"
					flag = 1
				else:
				 	score = self.getLineValue((self.budgetMaxInput),(self.budgetMaxInput)*self.scoringThresholdDown,price)
			...
			if(price >= budgetMean):
				...
				if(price <= searchBudget):
					...
					score = self.getLineValue((budgetMean + budgetStd/2),(budgetMean + budgetStd/2)*self.scoringThresholdUp,price)
					score = min(score,self.scoringMax)
			...
			#		price < budgetMean
				...
				if(price > searchBudget ):
					...
					score = self.getLineValue((budgetMean - budgetStd/2),(budgetMean - budgetStd/2)*self.scoringThresholdDown,price)
					score = min(score,self.scoringMax)
					text = "Property is within your budget"
